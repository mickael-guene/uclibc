...
__where_is_shmfs (void)
{
  ...
  if (__statfs (defaultmount, &f) == 0 && (f.f_type == SHMFS_SUPER_MAGIC_WITH_MMU || f.f_type == SHMFS_SUPER_MAGIC_WITHOUT_MMU))
    {
  ...
  while ((mp = __getmntent_r (fp, &resmem, buf, sizeof buf)) != NULL)
	...
	if (__statfs (mp->mnt_dir, &f) != 0 || (f.f_type != SHMFS_SUPER_MAGIC_WITH_MMU && f.f_type != SHMFS_SUPER_MAGIC_WITHOUT_MMU))
	  continue;
